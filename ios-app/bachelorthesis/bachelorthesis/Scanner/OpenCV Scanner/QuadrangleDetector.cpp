...
void QuadrangleDetector::detect_squares( const Mat& image )
{
...
double QuadrangleDetector::angle( Point pt1, Point pt2, Point pt0 )
{
...
void QuadrangleDetector::find_squares( const Mat& image, vector<vector<Point> >& squares )
{
  int thresh = 50, N = 11;
...
void QuadrangleDetector::find_largest_square(const vector<vector<Point> >& squares, vector<Point>& biggest_square)
{
  if (!squares.size()){
    // no squares detected
    return;
...
void QuadrangleDetector::draw_squares( const Mat& image, const vector<vector<Point> >& squares )
{
  for( size_t i = 0; i < squares.size(); i++ )
  {
    ...
    polylines(image, &p, &n, 1, true, Scalar(255,255,255), 1, LINE_AA);
  }
}

void QuadrangleDetector::draw_square( const Mat& image, const vector<Point>& square )
{
